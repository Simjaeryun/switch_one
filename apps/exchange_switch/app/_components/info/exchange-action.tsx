  ...
  return (
    ...
    >
            ...
            <div className="flex gap-2">
              {exchangeRates?.map((rate) => (
                <button
                  ...
                  className={cn(
                    "flex-1 cursor-pointer rounded-lg border-2 px-4 py-3 text-sm font-medium transition-all",
      ...
      <Controller
        ...
        name="orderType"
        render={({ field }) => {
          const isDisabled = form.watch("currency") === "";
          return (
        ...
        <Controller
          ...
          render={({ field }) => {
            ...
            return (
              <div
                className={cn(
                  "relative flex items-center overflow-hidden rounded-lg border-2 transition-colors",
                  isDisabled
                    ? "border-gray-200 bg-gray-100"
                    : "border-gray-200 bg-white focus-within:border-[#3CDC84]"
                )}
              >
                <input
                  type="number"
                  ...
                  className={cn(
                    "flex-1 border-0 bg-transparent px-4 py-3 text-lg font-semibold placeholder:text-gray-400 focus:outline-none",
                    isDisabled
                ...
                <div
                  className={cn(
                    "border-l px-4 py-3 text-sm font-medium",
                    isDisabled
                      ? "border-gray-200 bg-gray-100 text-gray-400"
                      : "border-gray-200 bg-gray-50 text-gray-700"
                  )}
                >
                  {form.watch("currency") || "통화"}
                </div>
              </div>
            );
          }}
      ...
      <button
        ...
        className={cn(
          "w-full cursor-pointer rounded-lg bg-[#f7931a] px-6 py-4 text-base font-semibold text-white shadow-lg transition-all duration-200 hover:scale-[1.02] hover:bg-[#e6801b] hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
        )}
      >
        환전하기
      </button>
